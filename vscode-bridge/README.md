# Codex Switchboard Bridge

This extension runs in every local VS Code window and watches
`~/.codex-switchboard/vscode-refresh.signal`. When Codex Switchboard updates
the signal, every window restarts its extension host without reloading the
window.

The URI `vscode://cuihongran.codex-switchboard-bridge/restart-extension-host`
